...
const Header = () => {
  ...
  return <header className="bg-white shadow-lg sticky top-0 z-50">
      ...
      <div className="text-white py-2 hidden sm:block" style={{
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center text-xs sm:text-sm">
            <div className="flex flex-col sm:flex-row items-center space-y-1 sm:space-y-0 sm:space-x-4">
              <div className="flex items-center">
                <Phone className="w-3 sm:w-4 h-3 sm:h-4 mr-1" />
                <span>(55) 3221-8035</span>
              </div>
              <div className="flex items-center">
                <Mail className="w-3 sm:w-4 h-3 sm:h-4 mr-1" />
                <span className="hidden md:inline">supercolchoes@yahoo.com.br</span>
                <span className="md:hidden">contato@supercolchoes</span>
              </div>
            </div>
      ...
      <div className="container mx-auto px-4 py-3 sm:py-4">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <Link to="/">
              <img src="/lovable-uploads/748faed7-8409-4ca0-a866-967259247f21.png" alt="SuperColchões" className="h-10 sm:h-12 w-auto" />
            </Link>
          </div>
          ...
          <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            ...
            <Button className="text-sm xl:text-base px-4 xl:px-6 flex items-center" style={{
            backgroundColor: '#3b3f3c'
          ...
          {/* Mobile menu button */}
          <button onClick={toggleMenu} className="lg:hidden p-2 rounded-md text-gray-700 hover:bg-gray-100">
            {isMenuOpen ? <X className="w-5 h-5 sm:w-6 sm:h-6" /> : <Menu className="w-5 h-5 sm:w-6 sm:h-6" />}
          </button>
        </div>

        {/* Mobile Navigation */}
        {isMenuOpen && <nav className="lg:hidden mt-4 pb-4 border-t border-gray-200">
